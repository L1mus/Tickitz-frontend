...
function MovieDetails() {
  const { id } = useParams();
  const { movieDetail, movieShowtime, loading, error } = useSelector(
    (state) => state.movies
  );
    ...
    <section id="movie-detail-container">
      ...
      <section className="relative z-10 -mt-90 px-5 md:px-20 lg:px-24">
        <section className="md:flex md:flex-row">
          <div>
            <img
              className="rounded-md shadow-lg md:mt-20 md:w-80"
              src={`${movieDetail?.poster}`}
              alt={movieDetail?.title}
            />
          </div>
              ...
              <section>
                ...
                <div className="flex flex-wrap gap-1 font-medium">
                  {movieDetail?.casts?.map((c, idx) => (
                    <p key={idx}>
                      {c.name}
                      {idx < movieDetail.casts.length - 1 ? ',' : ''}
                    </p>
                  ))}
                </div>
              </section>
      ...
      >
        ...
        <div className="mt-4 flex flex-col gap-2 md:flex-row md:justify-between md:gap-10">
          ...
          <section className="w-full">
            ...
            <div className="relative flex h-12 w-full items-center rounded-sm bg-gray-200 p-3">
              <img
            ...
            {(formData.time || formData.location) && (
              <p className="mt-1 text-left text-xs text-blue-500">
          ...
          <section className="my-6 flex w-full flex-col justify-start gap-4 px-5 md:flex-row md:gap-8 md:px-20 lg:px-24">
            {currentCinemas.map((cinema) => (
              <button
                ...
                className={`flex min-h-20 w-full rounded-md border bg-white py-6 transition-all duration-200 outline-none hover:shadow-md active:scale-95 md:w-1/4 ${selectedCinema === cinema.id ? 'border-blue-600 bg-blue-50 ring-2 ring-blue-600' : 'border-gray-300 hover:border-blue-500'}`}
              >
                <img
